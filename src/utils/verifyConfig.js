...
const REQUIRED_CLIENT_ID = "848194055619215360";
...
async function verifyConfig() {
	...
	if (!process.env.CLASH_OF_CLANS_API_KEY) {
		...
		process.exit(1);
	} else if (typeof process.env.CLASH_OF_CLANS_API_KEY !== "string") {
	...
	if (!process.env.CLASH_OF_CLANS_PLAYER_TAG) {
		...
		process.exit(1);
	} else if (typeof process.env.CLASH_OF_CLANS_PLAYER_TAG !== "string") {
